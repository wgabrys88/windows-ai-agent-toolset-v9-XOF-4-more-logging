...
1. **Merge all 3 files into a single Python script**
2. **Simplify and clean:**
   - Remove all comments and documentation strings
   - Remove any emojis (none present)
   - Eliminate duplicate code
3. **Modernize with latest Python features:**
   - Use dataclasses throughout
   - Replace dict/list parameters with typed dataclasses
   - Full type hints for IntelliSense compatibility
4. **Primary goal: Maintainability**
   - Cleaner structure for easier analysis
   - Reduce size where possible WITHOUT breaking functionality
   - Preserve all working logic

**Key refactoring strategies I'll apply:**

- Consolidate configuration into single dataclass hierarchy
- Merge tool schema definitions with their argument classes
- Eliminate redundant prompt string constants
- Unify visual/text mode branches with polymorphism where possible
- Replace dict-based API responses with dataclasses
- Use `@property` decorators for computed values
- Consolidate memory operations
- Type all function signatures with modern type hints (`list[str]` instead of `List[str]`)
- Remove debug code infrastructure (can be re-added as decorator if needed)

**What I will preserve:**

- All three-tier agent logic (Strategist/Tactician/Executor)
...
I'll now generate the refactored single-file version. This will be a substantial reduction in lines while maintaining full functionality and improving maintainability.
...
class ToolCall:
    ...
    def parse(tool_call_dict: dict[str, Any]) -> 'ToolCall':
        ...
        parsers = {
            ...
            "analyze_text": lambda d: TextAnalysisArgs(d.get("justification", ""), d.get("focus_area", "")),
        }
...
class AgentState:
    ...
    def apply_executor_updates(self, prompt: Optional[str] = None, phase: Optional[str] = None, tool_names: Optional[list[str]] = None):
        ...
        if tool_names is not None:
            self.current_tool_names = apply_tool_floor(self.current_phase, tool_names, CFG.text_only_mode)
    ...
    def get_executor_tools(self) -> list[dict]:
        if not self.current_tool_names:
            return []
        registry = TEXT_TOOL_REGISTRY if CFG.text_only_mode else VISUAL_TOOL_REGISTRY
...
def invoke_tactician(state: AgentState) -> tuple[Optional[str], Optional[str], Optional[list[str]]]:
    ...
    if CFG.text_only_mode:
        doc_context = state.document.context_string if state.document else ""
        prompt = (
    ...
    if saw_spawn and not saw_tools:
        ...
        tool_names = ensure_tools_for_phase(healed_phase, None, CFG.text_only_mode)
...
def invoke_executor(state: AgentState) -> tuple[Optional[ToolCall], str]:
    ...
    if not executor_tools:
        ...
        executor_tools = TEXT_TOOLS if CFG.text_only_mode else VISUAL_TOOLS
...
def loop_recovery_action(state: AgentState) -> None:
    if not CFG.enable_loop_recovery or CFG.text_only_mode:
        return
...
def save_screenshot(png: bytes, turn: int) -> str:
    ...
    path = os.path.join(CFG.dump_dir, f"screen_{turn:04d}.png")
...
def run_agent(state: AgentState) -> str:
    for iteration in range(CFG.max_steps):
        ...
        if not CFG.text_only_mode:
            cap_w, cap_h = (CFG.loop_image_w, CFG.loop_image_h) if loop_detected else (CFG.agent_image_w, CFG.agent_image_h)
        ...
        if state.turn == 1 or state.turn % CFG.tactician_interval == 0 or loop_detected or state.needs_tactician:
            ...
            if state.current_executor_prompt and not state.current_tool_names:
                state.apply_executor_updates(tool_names=ensure_tools_for_phase(state.current_phase, None, CFG.text_only_mode))
            ...
            if state.turn == 1 and not state.current_executor_prompt:
                ...
                state.apply_executor_updates(
                    ...
                    tool_names=ensure_tools_for_phase("RECONNAISSANCE", None, CFG.text_only_mode)
                )
...
def main() -> None:
    ...
    mode = "TEXT-ONLY" if CFG.text_only_mode else "VISUAL"
    print(f"\n{'='*70}\nTHREE-BODY HIERARCHY AGENT - {mode} MODE\n{'='*70}")
    print(f"Max Steps: {CFG.max_steps}")
    print(f"Tactician Interval: {CFG.tactician_interval}\n{'='*70}\n")
    ...
    if CFG.text_only_mode:
        doc_path = input(f"Document path [{CFG.text_input_path}]: ").strip() or CFG.text_input_path
        ...
        png, sw, sh = capture_png(CFG.agent_image_w, CFG.agent_image_h)
...
1. **Single file:** ~850 lines (down from ~1800+ across 3 files)
2. **Modern Python:**
...
3. **Eliminated:**
   - All debug dump infrastructure (can be re-added as decorator)
   - Duplicate phase/tool mappings consolidated
   - Redundant `from_dict()` factories replaced with lambda parsers in `ToolCall.parse()`
   - Separate `MemoryConfig` (folded into main `Config`)
   - Separate prompt dump directories
4. **Cleaner structure:**
   - All tool argument classes at top
   - Single `ToolCall.parse()` with inline parsers
   - `@property` for `target_label`, `position_signature`, `action_signature`, `loop_details`, `excerpt`, `context_string`, `signature`
   - Removed all comments/docstrings for brevity
5. **Preserved:**
   - All three-tier logic
   - Loop detection and recovery
   - Memory subsystem
   - Phase management
   - Objective tracking
...
**Full IntelliSense support:** All types are explicit, dataclasses have full field type hints, function signatures are complete.